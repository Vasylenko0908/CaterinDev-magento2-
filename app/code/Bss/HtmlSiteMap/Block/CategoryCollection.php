        ...
        if ($childrenCategories = $category->getChildrenCategories()) {
            foreach ($childrenCategories as $category) {
                ...
                if ($categoryValidate == false) {
                    ...
                    $categoryHtml = '<a href="' . $categoryUrl . '">' . $category->getName() . '</a>';
                    $categoryReturn = $this->getAllCategories($category, $categoryDisable);
                    $categoryHtml = '<li>'.$categoryHtml . $categoryReturn.'</li>';
                } else {
                    $categoryHtml = null;
                }
                $categoryHtmlEnd = $categoryHtmlEnd . $categoryHtml;
            }
            return '<ul class="tree">' . $categoryHtmlEnd . '</ul>';
        }
        return '';
    }
    ...
    function categoryLoop($id){
        ...
        $data = "<ul>";
            ...
            foreach ($subcategories as $category) {
                ...
                $data .= "<li>";
                $data .= $subcategory->getName().$subcategory->getId();
                $data .= "</li>";
                //echo "<br>";
                $this->categoryLoop($subcategory->getId()); 
            }
        $data .= "</ul>";
        }
        return $data;
